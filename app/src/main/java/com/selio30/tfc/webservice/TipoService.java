...
public class TipoService {
    private static final String TAG = "TIPO_CONTROLLER";
    private Context context;
    private View view;
    private Gson gson = new Gson();
    private VolleyViewModel volleyViewModel;

    public TipoService(Context context, View view) {
    ...
    public void create(Tipo tipo) {
        ...
        map.put("tipos", tipo.getTipos());

        JSONObject jsonObject = new JSONObject(map);

        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.POST, JSonConstants.URL_POST_TIPO, jsonObject, new Response.Listener<JSONObject>() {
            @Override
    ...
    public void read() {
        MutableLiveData<List<Tipo>> mutableLiveData = volleyViewModel.getTipos();

        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, JSonConstants.URL_GET_TIPO, null, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
    ...
    public void readParseResponse(JSONObject response) {
        try {
            if (response.getString("body") != null) {
                ...
                List<Tipo> tipoList = new ArrayList(Arrays.asList(gson.fromJson(body.toString(), Tipo[].class)));
                volleyViewModel.getTipos().setValue(tipoList);
            } else {
