...
public class ProductoAlmacenService {
    private static final String TAG = "PRODALMACEN_CONTROLLER";
    private Context context;
    private View view;
    private Gson gson = new Gson();
    private VolleyViewModel volleyViewModel;

    public ProductoAlmacenService(Context context, View view) {
     ...
     *
     * @param productoAlmacen
     */

    public void create(ProductoAlmacen productoAlmacen) {
        HashMap<String, String> map = new HashMap<>();
        map.put("idProducto", productoAlmacen.getId_producto());
        map.put("idAlmacen", productoAlmacen.getId_almacen());

        JSONObject jsonObject = new JSONObject(map);

        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.POST, JSonConstants.URL_POST_PRODUCTOALMACEN, jsonObject, new Response.Listener<JSONObject>() {
            @Override
    ...
    public void read() {
        MutableLiveData<List<ProductoAlmacen>> mutableLiveData = volleyViewModel.getProductoalmacens();

        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, JSonConstants.URL_GET_PRODUCTOALMACEN, null, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
    ...
    public void readParseResponse(JSONObject response) {
        try {
            if (response.getString("body") != null) {
                ...
                List<ProductoAlmacen> productoAlmacenList = new ArrayList(Arrays.asList(gson.fromJson(body.toString(), ProductoAlmacen[].class)));
                volleyViewModel.getProductoalmacens().setValue(productoAlmacenList);
            } else {
