...
public class UbicacionService {
    private static final String TAG = "UBICACION_CONTROLLER";
    private Context context;
    private View view;
    private Gson gson = new Gson();
    private VolleyViewModel volleyViewModel;

    public UbicacionService(Context context, View view) {
    ...
    public void create(Ubicacion ubicacion) {
        ...
        map.put("id", ubicacion.getId());
        map.put("idAlmacen", ubicacion.getId_almacen());
        map.put("name", ubicacion.getName());
        map.put("orden", ubicacion.getOrden());

        JSONObject jsonObject = new JSONObject(map);

        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.POST, JSonConstants.URL_POST_UBICACION, jsonObject, new Response.Listener<JSONObject>() {
            @Override
    ...
    public void read() {
        MutableLiveData<List<Ubicacion>> mutableLiveData = volleyViewModel.getUbicacions();

        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, JSonConstants.URL_GET_UBICACION, null, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
            ...
            public void onErrorResponse(VolleyError error) {
                ...
                if (error != null && error.networkResponse.statusCode == 404) {
                    ...
                    Log.d(TAG, "Error en la ejecucion de Volley: " + error.getMessage());
                }
            }
        });
    ...
    private void readParseResponse(JSONObject response) {
        try {
            if (response.getString("body") != null) {
                JSONArray body = response.getJSONArray("body");
                List<Ubicacion> ubicacionList = new ArrayList(Arrays.asList(gson.fromJson(body.toString(), Ubicacion[].class)));
                volleyViewModel.getUbicacions().setValue(ubicacionList);
            } else {
