...
public class LocalizacionService {
    private static final String TAG = "LOCALIZACION_CONTROLLER";
    private Context context;
    private View view;
    private Gson gson = new Gson();
    private VolleyViewModel volleyViewModel;

    public LocalizacionService(Context context, View view) {
    ...
    public void create(Localizacion localizacion) {
        ...
        map.put("id", localizacion.getId());
        map.put("ciudad", localizacion.getCiudad());
        map.put("provincia", localizacion.getProvincia());

        JSONObject jsonObject = new JSONObject(map);

        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.POST, JSonConstants.URL_POST_LOCALIZACION, jsonObject, new Response.Listener<JSONObject>() {
            @Override
    ...
    public void read() {
        MutableLiveData<List<Localizacion>> mutableLiveData = volleyViewModel.getLocalizacions();

        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, JSonConstants.URL_GET_LOCALIZACION, null, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
    ...
    public void readParseResponse(JSONObject response) {
        try {
            if (response.getString("body") != null) {
                ...
                List<Localizacion> localizacionList = new ArrayList(Arrays.asList(gson.fromJson(body.toString(), Localizacion[].class)));
                volleyViewModel.getLocalizacions().setValue(localizacionList);
            } else {
