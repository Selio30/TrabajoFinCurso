...
public class FormatoService {
    private static final String TAG = "FORMATO_CONTROLLER";
    private Context context;
    private View view;
    private Gson gson = new Gson();
    private VolleyViewModel volleyViewModel;

    public FormatoService(Context context, View view) {
    ...
    public void create(Formato formato) {
        ...
        map.put("id", formato.getId());
        map.put("formato", formato.getFormato());
        map.put("volumen", formato.getVolumen());

        JSONObject jsonObject = new JSONObject(map);

        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.POST, JSonConstants.URL_POST_FORMATO, jsonObject, new Response.Listener<JSONObject>() {
            @Override
    ...
    public void read() {
        MutableLiveData<List<Formato>> mutableLiveData = volleyViewModel.getFormatos();

        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, JSonConstants.URL_GET_FORMATO, null, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
    ...
    public void readParseResponse(JSONObject response) {
        try {
            if (response.getString("body") != null) {
                ...
                List<Formato> formatoList = new ArrayList(Arrays.asList(gson.fromJson(body.toString(), Formato[].class)));
                volleyViewModel.getFormatos().setValue(formatoList);
            } else {
